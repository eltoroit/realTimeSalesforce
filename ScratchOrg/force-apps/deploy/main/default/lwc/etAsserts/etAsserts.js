export default class ET_Asserts {
	...
	static hasData({ value, message }) {
		if (!["string", "number", "boolean"].includes(typeof value)) {
			if (!value) {
				...
				throw new Error(JSON.stringify({ value, message }, null, 2));
			}
		}
	}
	...
	static includes({ value, listValues, message }) {
		if (!Array.isArray(listValues)) {
			...
			throw new Error(JSON.stringify({ value, listValues, message }, null, 2));
		}
	...
	static assert({ trueValue, message }) {
		if (typeof trueValue !== "boolean") {
			...
			throw new Error(JSON.stringify({ trueValue, message }, null, 2));
		}
		if (!trueValue) {
